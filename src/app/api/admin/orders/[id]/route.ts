...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!z.string().uuid().safeParse(id).success) return NextResponse.json({ message: "Invalid order ID." }, { status: 400 });
    const order = await getAdminOrderDetail(id);
    if (!order) return NextResponse.json({ message: "Order not found." }, { status: 404 });
    return NextResponse.json({ order }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    console.error("Admin order details failed:", error);
    return NextResponse.json({ message: "Unable to load order details." }, { status: 500 });
  }
}
