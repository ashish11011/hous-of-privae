...
import { Heart, Home, Menu, Search, Scissors, ShoppingBag, Store, User, X } from "lucide-react";
...
export default function NavBar() {
  ...
  const goToStudio = () => {
    closeMenu();
    if (pathname === "/") {
      document.getElementById("privae-studio")?.scrollIntoView({ behavior: "smooth" });
      return;
    }
  ...
  return (
    <header className="sticky top-0 z-50 bg-white text-neutral-950">
      ...
      <div className="border-b border-neutral-900 bg-white">
        <div className="container mx-auto px-4 flex flex-col items-center justify-center py-2 md:py-3">
          <Link href="/" aria-label="Haus of Privae home">
            <span className="text-xl md:text-3xl font-medium tracking-[0.35em] text-neutral-950">
              HAUS OF PRIVAE
            </span>
          </Link>
      ...
      <nav className="border-b border-neutral-900 bg-white">
        <div className="container mx-auto px-4 flex items-center justify-between h-12 md:h-14 bg-white">
          <div className="flex items-center gap-4 md:gap-6">
            <button
              onClick={() => setIsMenuOpen((open) => !open)}
              className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-white text-neutral-950 hover:bg-neutral-100 transition-colors"
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <Link
              href="/my-wishlist"
              className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-white text-neutral-950 hover:bg-neutral-100 transition-colors relative"
              aria-label="Wishlist"
            >
              <Heart size={16} />
          ...
          <div className="absolute left-0 right-0 top-full z-50 border-b border-t border-neutral-900 bg-white shadow-sm animate-fade-in-up">
            <div className="container mx-auto px-6 py-8 grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12 bg-white">
              <div className="md:col-span-2">
                <Link href="/" onClick={closeMenu} className="nav-link inline-flex items-center gap-2 text-sm tracking-[0.15em] uppercase font-body text-neutral-700 hover:text-neutral-950">
                  <Home size={14} strokeWidth={1.5} /> Home
                </Link>
              </div>
              <MenuGroup title="Collections" links={collectionLinks} onClick={closeMenu} />
              <MenuGroup title="Shop By Category" links={categoryLinks} onClick={closeMenu} />
              <MenuGroup title="Discover" links={discoverLinks} onClick={closeMenu} />
              <MenuGroup
                title="Client Care"
                links={[
              ...
              <div className="md:col-span-2 border-t border-neutral-900 pt-5 mt-2">
                <Link href="/auth/login" onClick={closeMenu} className="text-sm tracking-[0.15em] uppercase font-body text-neutral-700 hover:text-neutral-950 transition-colors">
